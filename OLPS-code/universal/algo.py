...
class Algo(object):
    ...
    or weights method, which does it at once. Upper case letters stand for matrix and lower case for vectors (such as
    B and b for weights).
    """
    ...
    def step(self, x, last_b, history):
        """ Calculate new portfolio weights. If history parameter is omitted, step
        method gets passed just parameters `x` and `last_b`. This significantly
        increases performance.
        :param x: Last returns.
        :param last_b: Last weights.
        :param history: All returns up to now. You can omit this parameter to increase performance.
        """
        raise NotImplementedError('Subclass must implement this!')

    def _use_history_step(self):
        """ Use history parameter in step method """
        step_args = inspect.getfullargspec(self.step)[0]
    ...
    def weights(self, X, min_history=None, log_progress=True):
        ...
        for t, (_, x) in enumerate(X.iterrows()):
            ...
            # predict for t+1
            if use_history:
                history = X.iloc[:t + 1]  # historical relative prices
            ...
            # show progress by 10 percent
            if log_progress:
                tools.log_progress(t, len(X), by=10)
        return B

    @staticmethod
    def _split_index(ix, nr_chunks, freq):
        """ Split index into chunks so that each chunk except of the last has length divisible by freq. """
    ...
    def run(self, S, n_jobs=1, log_progress=True):
        ...
        :param S: Absolute stock prices. DataFrame with stocks in columns.
        :param log_progress: Log computation progress. Works only for algo with defined step method.
        ...
        # convert prices to relative price vector
        ...
        # if we are aggregating strategies, combine weights from strategies and use original assets
        if isinstance(S, ListResult):
    ...
    def run_subsets(self, S, r, generator=False):
        """ Run algorithm on all stock subsets of length r. Note that number of such tests can be very large.
    ...
    def run_combination(cls, S, **kwargs):
        """ Get equity of algo using all combinations of parameters. All values in lists
        specified in kwargs will be optimized. Return ListResult object, which is basically a wrapper of list of
        AlgoResult objects. It is possible to pass ListResult to Algo or run_combination again
        ...
        simple_params = {k: v for k, v in kwargs.items() if not isinstance(v, list)}
        ...
        for seq in itertools.product(*kwargs.values()):
            ...
            # run algo
            params.update(simple_params)
            params_to_try.append(params)
